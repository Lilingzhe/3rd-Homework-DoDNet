...
```
python train.py --train_list=list/MOTS/MOTS_train.txt --snapshot_dir=snapshots/dodnet --input_size=64,192,192 --batch_size=1 --num_gpus=1 --num_epochs=30 --start_epoch=0 --learning_rate=1e-1 --num_classes=2 --num_workers=0 --weight_std=True --random_mirror=False --random_scale=False --FP16=True
```

### 4. Evaluation
```
python evaluate.py --val_list=list/MOTS/MOTS_test.txt --reload_from_checkpoint=True --reload_path=snapshots/dodnet/MOTS_DynConv_checkpoint.pth --save_path=outputs/ --input_size=64,192,192 --batch_size=1 --num_gpus=1 --num_workers=2
```
